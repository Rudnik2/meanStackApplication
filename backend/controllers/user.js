...
exports.userLogin = (req,res,next)=>{
  ...
  .then(result=>{ //result of the compare operation
    if(!result){
...
exports.updateUser = (req,res,next)=>{
  ...
  User.updateOne({_id:req.params.id},{$set:{imagePath:url+"/images/"+req.file.filename}}).then(result=>{
...
exports.updateSummary = (req,res,next)=>{
  ...
  User.updateOne({_id:req.params.id},{$set:{summaryType:req.params.summaryType,summaryNotificationDate:req.params.summaryNotificationDate}}).then(result=>{
    if(result.matchedCount>0){
      res.status(200).json({message:"Update Succesfull",
    user:user});
    }else{
