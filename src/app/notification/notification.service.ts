    ...
    if (this.userIsAuthenticated) {
      this.authService
        ...
        .subscribe((userData) => {
          ...
          var difference =
            (now.getTime() - lastUpdate.getTime()) / (1000 * 3600 * 24); // gdy będzie większe od 1, to znaczy że 1 dzień temu ostatni raz się wyświetlił komunikat

          if (now.getUTCDate() == 1) {
            // jezeli jest 1 dzień nowego miesiąca
            if (difference >= 1) {
